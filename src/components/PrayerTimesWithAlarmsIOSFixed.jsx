...
const PrayerTimesWithAlarmsIOSFixed = () => {
  ...
  const isSimulator = () => {
    // Check for simulator user agent patterns
  ...
  // Load default location for testing/simulator
  const loadDefaultLocation = useCallback(() => {
    // Default to Mecca
    const defaultLat = 21.4225;
    const defaultLon = 39.8262;
    const defaultCity = 'Mecca';
    ...
    const times = calculateSunTimes(defaultLat, defaultLon, selectedDate, tz);
    setPrayerTimes(times);
    setError(null);
  }, [selectedDate]);

  // Get user's current location (iOS optimized with fallback)
  const getCurrentLocation = () => {
    ...
    if (!navigator.geolocation) {
      setError('Location services not available. Please enter a city manually.');
      setLoading(false);
      return;
    }

    // Set a shorter timeout for iOS
  ...
  useEffect(() => {
    ...
    const savedLocation = localStorage.getItem('savedPrayerLocation');
    if (savedLocation) {
      try {
        ...
        const tz = Intl.DateTimeFormat().resolvedOptions().timeZone;
        setTimezone(tz);
        const times = calculateSunTimes(lat, lon, selectedDate, tz);
        setPrayerTimes(times);
        setLocationAttempted(true);
      } catch (e) {
  ...
  useEffect(() => {
    if (location && cityName) {
      localStorage.setItem('savedPrayerLocation', JSON.stringify({
        lat: location.lat,
        lon: location.lon,
        city: cityName
      }));
  ...
  return (
    <div className="prayer-times-container">
      ...
      {!prayerTimes && !loading && !locationAttempted && (
        <div className="welcome-message">
          <p>Enter a city name or tap 📍 to use your location</p>
        </div>
      )}
...
export default PrayerTimesWithAlarmsIOSFixed;